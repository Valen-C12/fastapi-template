...
class CRUDRouter(Generic[ModelType, CreateSchemaType, UpdateSchemaType]):
    def __init__(
        ...
        *,
        ...
        dependencies: dict[str, list[params.Depends]] | None = None,
    ):
        self.crud = crud
        self.schema = schema
    ...
    def _register_routes(self):
        # Note: 保持装饰器模式以在自动生成的api文档中包含有body的类型信息
        if "create" not in self.disabled_endpoints:
            ...
            @self.router.post(
                ...
                dependencies=self.dependencies.get("create", []),
            )
            async def create(
                *,
        ...
        if "read_multi" not in self.disabled_endpoints:
            ...
            @self.router.get(
                ...
                dependencies=self.dependencies.get("read_multi", []),
            )
        ...
        if "read_one" not in self.disabled_endpoints:
            ...
            @self.router.get(
                "/{id}",
                response_model=self.schema,
                dependencies=self.dependencies.get("read_one", []),
            )
        ...
        if "update" not in self.disabled_endpoints:
            ...
            @self.router.put(
                "/{id}",
                response_model=self.schema,
                dependencies=self.dependencies.get("update", []),
            )
            async def update(
                *,
                db: AsyncSession = Depends(get_db),
        ...
        if "delete" not in self.disabled_endpoints:
            ...
            @self.router.delete(
                "/{id}",
                response_model=self.schema,
                dependencies=self.dependencies.get("delete", []),
            )
